const parser = {
  ...
  mergeTransactions(transactionArray) {
    ...
    transactionArray.forEach((t, index) => {
      ["cost", "amount"].forEach((v) => {
        mergedTransaction[v] =
          index === 0 ? t[v] : mergedTransaction[v] + t[v];
      });
    });
  ...
  categoriseTransactions(transactions) {
    ...
    transactions.forEach((t) => {
      ...
      categorisedTransactions["byYear"][
        year
      ...
      categorisedTransactions["byYear"][year]["byAsset"][asset].push(
        t
      );
      categorisedTransactions["byAsset"][asset]["byYear"][year].push(
        t
      );
    });
